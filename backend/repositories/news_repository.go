...
// 	var total int
// 	err := r.DB.QueryRow(`SELECT COUNT(*) FROM news`).Scan(&total)
// 	return total, err
// }
...
func (r *NewsRepository) SaveNews(items []models.NewsItem) error {
    ...
    // Добавили image_url и $6
    stmt, err := r.DB.Prepare(`
        INSERT INTO news (title, link, published_at, source, description, image_url)
        VALUES ($1, $2, $3, $4, $5, $6)
        ON CONFLICT (link) DO NOTHING
    `)
    if err != nil {
    ...
    for _, item := range items {
        if _, err := stmt.Exec(
            ...
            item.Source,
            item.Description,
            item.ImageURL, // Передаем картинку в БД
        ); err != nil {
            return err
        }
    }
...
func (r *NewsRepository) GetAllNews() ([]models.NewsItem, error) {
    // Добавили image_url в выборку
    rows, err := r.DB.Query(`
        SELECT id, title, link, published_at, source, description, image_url
        FROM news
        ORDER BY published_at DESC
    `)
    if err != nil {
