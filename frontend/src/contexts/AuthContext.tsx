...
// Добавляем интерфейс для данных дашборда
// (типы можно вынести в отдельный файл types.ts для чистоты)
...
interface AuthContextType {
  user: User | null;
  token: string | null;
  dashboardData: DashboardData | null; // <-- Новое поле
  login: (email: string, password: string) => Promise<void>;
  loginWithToken: (token: string) => Promise<void>;
  register: (username: string, email: string, password: string) => Promise<void>;
  logout: () => Promise<void>;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [user, setUser] = useState<User | null>(null);
  const [token, setToken] = useState<string | null>(localStorage.getItem('token'));
  const [dashboardData, setDashboardData] = useState<DashboardData | null>(null); // <-- Новое состояние
  const [isLoading, setIsLoading] = useState(true);

  // Вспомогательная функция для загрузки дашборда
    ...
    const initAuth = async () => {
      ...
      if (storedToken) {
        try {
          ...
          // Грузим дашборд при успешной проверке токена
        ...
        if (refreshToken) {
          try {
            ...
            // Грузим дашборд после успешного рефреша
            await fetchDashboardData();
          } catch {
            localStorage.removeItem('token');
            localStorage.removeItem('refresh_token');
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      // Запрашиваем данные дашборда СРАЗУ после логина
      await fetchDashboardData();
  ...
  const logout = async () => {
    try {
      ...
      setToken(null);
      setUser(null);
      setDashboardData(null); // <-- Очищаем данные дашборда при выходе
      toast.info('Logged out successfully');
    }
  };
  ...
  return (
    <AuthContext.Provider value={{ 
      user, 
      token, 
      dashboardData, // <-- Прокидываем в провайдер
      login, 
      loginWithToken, 
      register, 
      logout, 
